...
import org.overturetool.vdmj.messages.rtlog.RTMessage;
...
import org.overturetool.vdmj.messages.rtlog.RTThreadMessage;
...
public class NextGenRTLogger {
	...
	private String getClassName(OperationValue operation) {
		...
		{
			return operation.getSelf().type.name.name;
		}
		else
		{
			return operation.classdef.getName();
		}		
	}
	...
	private NextGenObject getObject(Integer id)
	{
	...
	{		
		NextGenObject o = new NextGenObject(object.objectReference, getClassDefinition(object.type.name.name) ,cpuMap.get(cpuResource.getNumber()));		
	...
	{	
		...
		{
			classDef = getClassDefinition(obj.type.name.name);
